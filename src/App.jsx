...
function App() {
  ...
  const exchangeRate = currencyInfo[to] || 1;
  return (
    <div
        ...
        <div className="w-full max-w-xl mx-auto border border-gray-60 rounded-lg p-8 backdrop-blur-sm bg-white/30">
          ...
          >
            ...
            <div className="relative w-full h-0.5">
              <button
                type="button"
                className="absolute left-1/2 -translate-x-1/2 -translate-y-1/2 border-2 border-white rounded-md bg-violet-600 text-white px-2 py-0.5"
                onClick={swap}
              >
                Swap
              </button>
          ...
          {/* Display the exchange rate as a disclaimer */}
          <div className="mt-4 text-center text-white font-bold">
            <p>1 {from.toUpperCase()} = {exchangeRate} {to.toUpperCase()}</p>
          </div>
        </div>
      </div>
